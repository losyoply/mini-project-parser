...
{
    BTNode *node = or_expr();
    if(node->data!=ID&&node->data!=INCDEC) err(1);
    check=1;
    ...
    if (root != NULL)
    {
        switch (root->data)
        {
            case ID:
                ...
                for(int i=0;i<sbcount;i++) if(strcmp(root->lexeme,table[i].name)==0) address = i*4;
            ...
            case ASSIGN:
                ...
                address = 0;
                for(int i=0;i<sbcount;i++)if(strcmp(root->left->lexeme,table[i].name)==0)address = i*4;
